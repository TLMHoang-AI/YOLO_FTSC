#!/usr/bin/env python3
"""Train the seed-42 P2-only full-resolution NATTEN kernel-3 screen."""
...
VARIANT = "nat_p2_k3"
CONFIG = ROOT / "models_related/models_config/yolov8/levir/yolov8n_p2_fpn_only_nat_k3.yaml"
...
def model_for(variant: str, pretrained: str):
    ...
    from ultralytics.nn.modules import NATBlock

    model = YOLO(CONFIG)
    model.load(pretrained, smart_transfer=True)
    attention = model.model.model[19]
    head = model.model.model[20]
    if not isinstance(attention, NATBlock):
        raise TypeError(f"Layer 19 resolved to {type(attention).__name__}")
    if attention.num_heads != 4 or attention.attn.kernel_size != (3, 3):
        raise ValueError("Expected NATBlock with four heads and a 3x3 neighborhood")
...
def write_manifest(model, variant: str, run_dir: Path, args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "kernel_size": list(attention.attn.kernel_size),
        "num_heads": attention.num_heads,
        "channels": attention.c,
        "detect_from": head.f,
        ...
        "amp": True,
        "nms_iou": 0.5,
        "note": "Full-resolution NAT mechanism screen; not a KV-compression connectivity control.",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project", type=Path, default=ROOT / "runs/levir_yolov8n_p2_nat_k3")
    ...
    parser.add_argument("--hf-repo-id", default="duyle2408/levir-yolov8n-p2-nat-k3-seed42")

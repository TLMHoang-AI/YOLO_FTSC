#!/usr/bin/env python3
"""Train, evaluate, and verify-upload seed-42 P2 channel-descriptor variants."""
...
VARIANTS = {
    "gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_cbam_channel_only.yaml",
    "gmp": CONFIG_ROOT / "yolov8n_p2_fpn_only_cbam_channel_gmp.yaml",
    "gap_gmp": CONFIG_ROOT / "yolov8n_p2_fpn_only_cbam_channel_avgmax.yaml",
}
...
def model_for(variant: str, pretrained: str):
    ...
    if not isinstance(attention, ChannelAttention) or attention.descriptor != {"gap": "avg", "gmp": "max", "gap_gmp": "avg_max"}[variant]:
        raise TypeError(f"{variant}: ChannelAttention descriptor did not resolve")
    if head.f != [19] or head.stride.tolist() != [4.0]:
        raise ValueError(f"{variant}: expected ChannelAttention -> Detect stride [4.0], got {head.f}, {head.stride.tolist()}")
    return model
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project", type=Path, default=ROOT / "runs/levir_yolov8n_p2_channel_descriptor")

...
def main():
    ...
    # Let's visualize the target that originally had a very low ratio (e.g. GF1_WFV2_E118.9_N24.3_20200710_L2A0004922278_11264_10240.png at box [87, 377, 93, 388])
    target_img_name = 'GF1_WFV3_E120.2_N22.2_20200710_L2A0004922264_7168_5120.png'
    target_box = [319, 464, 331, 478]
    
    print("Finding target image for visualization...")
    with torch.no_grad():
        for batch in dataloader:
            ...
            axes[0].set_title("Raw Image (Red Box: Target <20px)")
            axes[0].axis("off")
            
            # 2. Original P2 heatmap
            ...
            axes[1].set_title("Original P2 Feature Map (Diluted)")
            axes[1].axis("off")
            
            # 3. Enhanced P2 heatmap
            ...
            axes[2].set_title("Enhanced P2 Feature Map (Activated)")
            axes[2].axis("off")
            
            plt.tight_layout()
            out_file = "docs/reports/p2_enhanced_activation_comparison.png"
            fig.savefig(out_file, dpi=300)
            plt.close()
            print(f"Saved enhanced activation comparison plot to {out_file}")
            break

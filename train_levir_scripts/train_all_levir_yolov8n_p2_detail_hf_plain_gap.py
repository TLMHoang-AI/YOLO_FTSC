#!/usr/bin/env python3
"""Train/evaluate P2-only plain and GAP controls for HF attenuation and masked detail."""
...
workflow.EXPERIMENT = "levir_yolov8n_p2_detail_hf_plain_gap"
workflow.HF_REPO = "duyle2408/levir-yolov8n-p2-detail-hf-plain-gap"
workflow.VARIANTS = {
    "hf_plain": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml",
    "hf_gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_cbam_channel_only.yaml",
    "detail_plain": CONFIG_ROOT / "yolov8n_p2_fpn_only_masked_detail_plain.yaml",
    "detail_gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_masked_detail_gap.yaml",
}
...
def model_for(variant: str, pretrained: str):
    ...
    from ultralytics.nn.modules import ChannelAttention, Detect, MaskedP2DetailReconstruction

    layers = model.model.model
    head = layers[-1]
    if not isinstance(head, Detect) or head.stride.tolist() != [4.0]:
        raise ValueError(f"{variant}: expected P2-only Detect stride [4], got {type(head).__name__} {head.stride}")
    if variant == "hf_plain" and head.f != [18]:
        raise ValueError(f"{variant}: unexpected Detect inputs {head.f}")
    if variant == "hf_gap" and (not isinstance(layers[19], ChannelAttention) or head.f != [19]):
        raise ValueError(f"{variant}: GAP layer/Detect inputs did not resolve")
    if variant == "detail_plain" and (not isinstance(layers[19], MaskedP2DetailReconstruction) or head.f != [19]):
        raise ValueError(f"{variant}: masked detail layer/Detect inputs did not resolve")
    if variant == "detail_gap" and (
        not isinstance(layers[19], MaskedP2DetailReconstruction)
        or not isinstance(layers[20], ChannelAttention)
        or head.f != [20]
    ):
        raise ValueError(f"{variant}: masked detail + GAP graph did not resolve")
    return model
